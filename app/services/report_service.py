...
class ReportService:
    ...
    def calculate_statistics(self, registrations: List[WorkdayRegistration]) -> Dict[str, Any]:
        """
        ...
        stats["by_type"] = {
            wt: {
                ...
                "percentage": (
                    round((type_counts[wt] / len(registrations)) * 100, 1) if registrations else 0
                ),
            }
        ...
        stats["by_location"] = {
            loc: {"count": count, "percentage": round((count / len(registrations)) * 100, 1)}
            for loc, count in location_counts.items()
        }

        stats["by_day_of_week"] = {
            self._get_day_name(day): count for day, count in day_counts.items()
        }
        ...
        stats["time_patterns"] = {
            ...
            "average_hours_per_day": (
                round(stats["total_hours"] / stats["total_days"], 2)
    ...
    def export_to_json(self, report: WeeklyReport, include_statistics: bool = True) -> str:
        """
    ...
    def format_for_telegram(self, report: WeeklyReport, include_details: bool = True) -> str:
        """
        ...
        if include_details and report.registrations:
            ...
            if stats["time_patterns"]["average_hours_per_day"] > 0:
                ...
                message += (
                    f"• Hora inicio más temprana: {stats['time_patterns']['earliest_start']}\n"
                )
                message += f"• Hora fin más tardía: {stats['time_patterns']['latest_end']}\n"
                message += (
                    f"• Promedio horas/día: {stats['time_patterns']['average_hours_per_day']:.1f}h"
                )

        return message

    def generate_monthly_summary(
        self, registrations: List[WorkdayRegistration], year: int, month: int
    ) -> Dict[str, Any]:
        """
        ...
        month_regs = [
            reg for reg in registrations if reg.date.year == year and reg.date.month == month
        ]
