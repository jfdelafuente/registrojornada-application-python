...
from DiaValidator import dia_validate, validar_dia
...
vive_logger = setup_logger(
    name="ViveOrange", log_file=str(settings.logs_dir / "vive_orange.log"), level=logging.INFO
)
...
def info_handler(message):
    ...
    try:
        ...
        container.notification_service.send_info(
            "Generando informe semanal...", chat_id=message.chat.id
        )
        container.notification_service.send_message(mensaje, chat_id=message.chat.id)

    except RegistroJornadaException as e:
        # Handle application errors
        error_msg = container.error_handler.handle_exception(
            e, {"user": message.chat.username, "command": "/info"}
        )
        ...
        container.notification_service.send_message(
            "❌ Error inesperado al generar informe. Revisa los logs.", chat_id=message.chat.id
        )
...
def infop_handler(message):
    ...
    try:
        ...
        container.notification_service.send_info(
            "Generando informe semana anterior...", chat_id=message.chat.id
        )
        container.notification_service.send_message(mensaje, chat_id=message.chat.id)
        ...
        # Handle application errors
        error_msg = container.error_handler.handle_exception(
            e, {"user": message.chat.username, "command": "/infop"}
        )
        container.notification_service.send_message(error_msg, chat_id=message.chat.id)
...
def day_handler(message):
    ...
    try:
        ...
        if registrar:
            ...
            container.notification_service.send_success(
                "Operación Realizada", chat_id=message.chat.id
            )
            ...
            bot.register_next_step_handler_by_chat_id(
                message.chat.id, day_handler_teletrabajo, dia_registro
            )

    except RegistroJornadaException as e:
        error_msg = container.error_handler.handle_exception(
            e, {"user": message.chat.username, "command": "/dia", "day": day}
        )
        container.notification_service.send_message(error_msg, chat_id=message.chat.id)
        ...
        container.notification_service.send_message(
            "❌ Error inesperado al procesar la fecha. Revisa el formato.", chat_id=message.chat.id
        )
...
def day_handler_teletrabajo(message, day):
    ...
    try:
        if message.text.upper() == "Y":
            ...
            container.notification_service.send_success(
                "Operación Realizada (Forzada)", chat_id=message.chat.id
            )
            container.notification_service.send_message(msg, chat_id=message.chat.id)
        else:
            # Cancel operation
            container.notification_service.send_warning(
                "Operación Cancelada", chat_id=message.chat.id
            )
            container.notification_service.send_help_message(chat_id=message.chat.id)

    except RegistroJornadaException as e:
        error_msg = container.error_handler.handle_exception(
            e, {"user": message.chat.username, "command": "/dia (forced)", "day": day}
        )
        container.notification_service.send_message(error_msg, chat_id=message.chat.id)
        ...
        container.notification_service.send_message(
            "❌ Error inesperado al registrar jornada.", chat_id=message.chat.id
        )
